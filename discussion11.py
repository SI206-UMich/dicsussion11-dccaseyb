...
def create_patients_table(cur, conn):
    ...
    #THe above line deletes everything and restarts, not always necessary
...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in range(len(json_data)):
        cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES (?,?,?,?,?,?)",(i + 1, json_data[i]['name'], species.index(json_data[i]['species']), json_data[i]['age'], json_data[i]['cuteness'], json_data[i]['aggressiveness']))
    conn.commit()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    output_list = []
    cur.execute("SELECT * FROM Patients WHERE Aggressiveness < 10")
    for row in cur:
